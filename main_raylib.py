...
FONT = load_font_ex("RobotoMonoNerdFont-Medium.ttf", 20, None, 0)
while not window_should_close():
    ...
    draw_text_ex(FONT, search, Vector2(5, 5), 20, 0, WHITE)
    i = 0
    for torrent in torrents:
        if torrent.name.lower().__contains__(search):
            draw_text_ex(
                FONT, torrent.name[:100], Vector2(5, 5 + 20 + i * 20), 20, 0, WHITE
            )
            i = i + 1
    end_drawing()
